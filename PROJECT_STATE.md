...
| --- | --- | --- | --- | --- |
| Publish the complete role-aware mobile CRM inside canonical `/legacy-crm` | `codex/mobile-ui-navigation-09261603-09261811` / #248 | Codex | Implemented and verified: the dedicated phone shell uses canonical authenticated APIs, role-scoped OWNER/MANAGER/CONSULTANT/INSTALLER data, iPhone safe areas, keyboard-safe form scrolling, and an explicit full-desktop handoff; 382 tests, TypeScript, production build, diff check, PR CI, and Codex Bridge pass | Review the phone UI with authenticated role accounts, then merge through the normal release workflow |
...
## 2026-09-26 handoff — dedicated mobile CRM in the canonical workspace

- Scope: PR #248 adds a phone-only interface directly to authenticated `/legacy-crm`; the existing desktop document remains the underlying full CRM and can be opened explicitly with the mobile `Открыть полную desktop CRM` action.
- Mobile navigation: `Сегодня`, role-appropriate `Лиды`, `Проекты` / `Мои работы`, `Календарь`, and `Ещё`. Owner and Manager can list, open, create, and edit leads. Consultant and Installer use their existing server-scoped assignments; Installer jobs come from `/api/v1/installer-jobs/my`.
- Data and access: the interface reuses the existing authenticated Leads, Projects, Consultations, Installer Jobs, legacy field-work state, logout, and PostgreSQL-backed APIs. It introduces no second database, browser business-data store, or duplicate backend.
- Phone behavior: the shell includes top and bottom iPhone safe-area insets, 44px minimum touch targets, 16px form controls, horizontal-overflow protection, an independently scrolling content surface, bottom scroll padding, and focused-field centering for the on-screen keyboard.
- Repair during final verification: the previously pushed mobile module contained escaped template-literal delimiters and an escaped context interpolation, so the module initially failed compilation and the emitted browser script would still have been invalid after the delimiter-only repair. Both layers were corrected. The PWA keeps its `/` start URL so the existing role-aware login destination remains authoritative.
- Verification: 382/382 automated tests passed; `npm run typecheck` passed; `npm run build` completed all 109 static pages and the `/legacy-crm` dynamic route; `git diff --check` passed. GitHub CI and Codex Bridge both passed on implementation head `272ece4`. No production customer, project, consultation, employee, or authentication record was read or changed.
- Branch / PR: `codex/mobile-ui-navigation-09261603-09261811` / #248 (`https://github.com/zufarataev-code/Rolan-PRO-CRM/pull/248`).
- Blocker: authenticated visual acceptance on real iPhone-sized viewports remains required before merge; this task does not authorize deployment from the PR branch.
- Next action: review the mobile UI as Owner/Manager/Consultant/Installer without changing customer data, merge after approval, and deploy only from `main`.
