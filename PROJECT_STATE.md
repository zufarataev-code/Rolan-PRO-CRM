...
- Verification: 219 automated tests pass, including the real legacy script compilation, immutable lead-intent guards, monthly service attribution, cross-sell separation, proposal propagation, and canonical Project persistence. Local Prisma CLI did not complete on this host, so Prisma validation/generation, TypeScript, production build, and GitHub CI must run in the clean CI environment on the new head.
