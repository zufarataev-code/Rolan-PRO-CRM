...
| --- | --- | --- | --- | --- |
| Full visual architecture audit of every owner CRM section, workspace, modal, and field | `fix/full-crm-visual-architecture-audit` | Codex | In progress after owner reported that the normal Projects kanban still clips the final stages despite the earlier fullscreen-only fix | Inventory all visible routes and dialogs, audit desktop/mobile overflow in the authenticated production shell, repair shared layout rules, add coverage, then release and production-verify |
