...
| --- | --- | --- | --- | --- |
| Repair live Facebook Messenger lead capture before slot loading | `fix/facebook-messenger-advisory-lock` | Codex | Release-ready; production exposed Prisma `P2010` because `pg_advisory_xact_lock` returns PostgreSQL `void` through `$queryRaw`; changed to `$executeRaw`; 289 tests, TypeScript, and production build pass locally | Merge through PR, deploy from `main`, verify the signed production endpoint without creating a test lead, then repeat the live Messenger booking |
