...
| --- | --- | --- | --- | --- |
| Publish the complete role-aware mobile CRM inside canonical `/legacy-crm` | `codex/mobile-ui-navigation-09261603-09261811` / #248 | Codex | Implemented and locally verified: the dedicated phone shell uses canonical authenticated APIs, role-scoped OWNER/MANAGER/CONSULTANT/INSTALLER data, iPhone safe areas, keyboard-safe form scrolling, and an explicit full-desktop handoff; 382 tests, TypeScript, production build, and diff check pass | Let PR #248 CI verify the pushed head, review the phone UI with authenticated role accounts, then merge through the normal release workflow |
...
- Verification: 382/382 automated tests passed; `npm run typecheck` passed; `npm run build` completed all 109 static pages and the `/legacy-crm` dynamic route; `git diff --check` passed. No production customer, project, consultation, employee, or authentication record was read or changed.
- Branch / PR: `codex/mobile-ui-navigation-09261603-09261811` / #248 (`https://github.com/zufarataev-code/Rolan-PRO-CRM/pull/248`).
- Blocker: authenticated visual acceptance on real iPhone-sized viewports and GitHub CI remain required before merge; this task does not authorize deployment from the PR branch.
- Next action: wait for PR #248 checks, review the mobile UI as Owner/Manager/Consultant/Installer without changing customer data, merge after approval, and deploy only from `main`.
