...
| --- | --- | --- | --- | --- |
| Show canonical Messenger consultations inside the active `/legacy-crm` calendar | `fix/messenger-consultations-calendar-20260924` | Codex | In progress; production booking exists and SMS was sent, but the legacy calendar only reads `LegacyWorkspace.payload.orders` | Load role-scoped PostgreSQL consultations through the existing API, render them in the active calendar without copying them into legacy storage, verify, merge, deploy, and confirm the live booking is visible |
