...
| --- | --- | --- | --- | --- |
| Make Quick Project Entry compact, defer installer scheduling to production, and restore clear Proposal formation | `fix/quick-entry-proposal-workflow` | Codex | In progress after owner reported the R-260925-015 quick-entry card is stretched, installer/date assignment appears too early, and Proposal formation is blocked | Reproduce the live Project state read-only, separate commercial Proposal inputs from later installation scheduling, compact the service card, add coverage, release, and production-verify without changing the customer record |
