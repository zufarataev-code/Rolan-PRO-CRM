...
| --- | --- | --- | --- | --- |
| Fix cramped project/KP/cut-sheet workspaces and eliminate full-screen redraws after each edit | `fix/legacy-project-modal-stability` | Codex | Implemented and locally verified: desktop workspaces are wide, quick-project fields use a readable grid, proposal readiness no longer depends on installation scheduling, edits refresh only the active modal body with scroll preservation, full-screen kanban fits seven stages, and cut-sheet geometry keeps a readable minimum scale; `npm test` 365/365, `npm run typecheck`, `npm run build`, and `git diff --check` pass | Commit, open PR, merge after CI, deploy production, and visually verify the live CRM without changing customer data |
