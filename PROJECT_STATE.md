...
| --- | --- | --- | --- | --- |
| Fix cramped project/KP/cut-sheet workspaces and eliminate full-screen redraws after each edit | `fix/legacy-project-modal-wide-cascade` | Codex | Core fix deployed in PR #224; production verification found the generic 58rem workspace rule still won the CSS cascade, so the wide desktop override is being made more specific while retaining the mobile layout | Test the cascade regression, deploy the follow-up, and visually verify the live CRM without changing customer data |
