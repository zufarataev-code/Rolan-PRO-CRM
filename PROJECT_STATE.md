...
| --- | --- | --- | --- | --- |
| Repair CRM workspace architecture, false КП blocking, and employee creation feedback | PR #227 deployed as `2c48b45`; follow-up `fix/legacy-crm-proposal-status-copy` | Codex | Core fix is production-verified: the live measurement workspace is 1520px with no modal overflow; R-260925-014 uses its 438.55 sqft measurement basis, ignores the stale empty quick draft, reports the calculation ready, and exposes the КП action; employee creation now surfaces canonical API errors and rejects missing server roles. Final follow-up removes the stale `заблокировано` label from the collapsed documents tile | Merge/deploy the wording follow-up, verify the live tile says `проверить`, then record final completion |
